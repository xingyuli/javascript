...
Growler = (function() {
	...
	var IE = (-1 != navigator.appName.indexOf('Microsoft')) ? parseFloat(navigator.appVersion.split("MSIE ")[1]) || 0 : 0;
	...
	function createNotice(growler, msg, options){
		...
		var notice;
		if (opt.className != ""){
			notice = $(Dom.create('div', {'class' : opt.className, 'className' : opt.className}, {'display' : 'block', 'opacity' : 0}));
		} else {
			notice = $(Dom.create('div', {'class' : 'Growler-notice', 'className' : 'Growler-notice'}, {'display' : 'block', 'opacity' : 0}));
		}
		if (opt.sticky){
			var noticeExit = $(Dom.create('div', {'class' : 'Growler-notice-exit', 'className' : 'Growler-notice-exit'})).update('&times;');
			noticeExit.observe("click", function(){ removeNotice(notice, opt); });
			notice.appendChild(noticeExit);
		}
		notice.appendChild($(Dom.create('div', {'class' : 'Growler-notice-head', 'className' : 'Growler-notice-head'})).update(opt.header));
		notice.appendChild($(Dom.create('div', {'class' : 'Growler-notice-body', 'className' : 'Growler-notice-body'})).update(msg));
		growler.appendChild(notice);
	...
	clazz.prototype = {
		...
		initialize : function(options){
			...
			options = options || {};
			Object.extend(opt, options);
			this.growler = $(Dom.create('div', {'class' : 'Growler', 'className' : 'Growler', 'id' : 'Growler'}));
			this.growler.setStyle({ position: "absolute", padding: "10px", "width": opt.width, "z-index": "9000" });
			
			Event.observe(window, 'scroll', function() {
				$('Growler').setStyle({'bottom' : 0 - Dom.ViewPort.getScroll().y});
			});
			...
			if (this.growler.parentNode) {
				this.growler.parentNode.replaceChild(document.body, this.growler);
			}
